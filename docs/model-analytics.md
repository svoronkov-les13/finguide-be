...
Текущий переходный стенд уже отдаёт часть расчётов из real Spring Boot backend с embedded H2. Swagger UI реального сервиса: `http://66.42.121.18/finguide-api/swagger-ui.html`; mock Swagger остаётся только для сверки старых ответов.
