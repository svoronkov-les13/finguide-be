...
Используем **Java 21 + Spring Boot 3 + Keycloak + PostgreSQL** как модульный монолит с подходом «сначала контракт». Текущий переходный стенд уже поднят как real Spring Boot backend с embedded H2 и публичным Swagger UI по адресу `http://66.42.121.18/finguide-api/swagger-ui.html`; legacy mock Swagger остаётся только для сравнения во время миграции.
